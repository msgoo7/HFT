...
insert into appmaster.client (id, first_name, phone, email, account_status, account_validity, general_risk_template ,product_risk_template , client_type, updated_by, effective_timestamp)
select distinct on(primaryclientcode) primaryclientcode, name,'','',0, now() + interval '1 year','Default_CGT1' ,'Default_PT' , 1, 'SUPERADMIN', now()
...
(exchange, segment, id, ucc, participant_type, participant_code, account_type, is_ucc_inherited, updated_by, effective_timestamp , pan )
select
...
clientcodedetail.accounttype, 'f','SUPERADMIN', now() , clientcodedetail.pan from childparentcodemapping
